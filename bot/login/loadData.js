...
module.exports = async function (api, createLine) {
	...
	if (api && global.GoatBot.config.database.autoSyncWhenStart == true) {
		...
		try {
			...
			const allThreadInfoInbox = await api.getThreadList(9999999, null, 'INBOX');
const allThreadInfoPending = await api.getThreadList(9999999, null, 'PENDING');
const allThreadInfo = [...allThreadInfoInbox, ...allThreadInfoPending];
